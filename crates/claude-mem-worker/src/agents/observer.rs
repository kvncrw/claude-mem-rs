...
impl AgentRunner {
    ...
    async fn run(
        ...
        match self.provider.as_str() {
            ...
            "claude" | "claude-cli" | "claude_code" | "claude-code" => {
                self.run_claude(prompt, model_id).await
            }
            "gemini" | "gemini-cli" => self.run_gemini(prompt, model_id).await,
    ...
    async fn run_claude(
        ...
        let args = std::env::var("CLAUDE_MEM_CLAUDE_ARGS").unwrap_or_else(|_| "-p".to_owned());
    ...
    async fn run_gemini(
        ...
        let api_key = env_non_empty("CLAUDE_MEM_GEMINI_API_KEY")
            .or_else(|| env_non_empty("GEMINI_API_KEY"))
            .ok_or_else(|| ObserverError::Agent("Gemini API key is not configured".into()))?;
        let model = model_id
            .clone()
...
fn parse_provider_text_and_session(raw: &str) -> (String, Option<String>) {
    let Ok(value) = serde_json::from_str::<Value>(raw) else {
        return (raw.to_owned(), None);
    };
    let memory_session_id = value
        .get("session_id")
