...
pub enum ValidateWorkerPidStatus {
    ...
    Alive { pid: u32, port: u16, started_at: String },
}
...
pub fn validate_worker_pid_file<P: AsRef<Path>>(
    ...
    let text = match std::fs::read_to_string(path) {
        Ok(t) => t,
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return ValidateWorkerPidStatus::Missing,
        Err(e) => return ValidateWorkerPidStatus::Invalid(format!("IO error reading PID file: {e}")),
    };
